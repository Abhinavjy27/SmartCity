...
def evaluate_energy_accuracy(sample_size: int = 150000):
    ...
    # 1. Hourly Ground Truth Means
    hourly_actual = df.groupby('Hour')['Global_active_power'].mean()
    mean_baseline = df['Global_active_power'].mean()

    # 2. Predicted Diurnal Weights vs Ground Truth Normalized Values
    actual_weights = np.array([hourly_actual[h] / mean_baseline for h in range(24)])
    ...
    mae = mean_absolute_error(actual_kw, predicted_kw)
    rmse = math.sqrt(mean_squared_error(actual_kw, predicted_kw))
    r2 = r2_score(actual_kw, predicted_kw)
    mape = np.mean(np.abs((actual_kw - predicted_kw) / actual_kw)) * 100.0
    ...
    print("-" * 50)
    for h in range(24):
        err = abs(actual_kw[h] - predicted_kw[h]) / actual_kw[h] * 100.0
